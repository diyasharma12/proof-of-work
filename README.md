# 🚀 Proof of Work

My open-source contributions to public organizations.

**Total PRs**: 15 | **Merged PRs**: 1 | **Total Issues**: 3 | **Closed Issues**: 1


## Organization: fossasia

- PRs: 15
- Issues: 3

### Pull Requests

| Repository | PR Title | Status | Created At | Merged At | Link |
|------------|----------|--------|------------|-----------|------|
| scrum_helper | fix prevent generate button text overflow | Open | 2026-03-22 | - | [PR #478](https://github.com/fossasia/scrum_helper/pull/478) |
| scrum_helper | Fix: Correct project name in documentation | Open | 2026-03-21 | - | [PR #473](https://github.com/fossasia/scrum_helper/pull/473) |
| scrum_helper | Add storage for scrum report configuration | Open | 2026-03-11 | - | [PR #446](https://github.com/fossasia/scrum_helper/pull/446) |
| scrum_helper | Fix/UI error overflow | Closed | 2026-03-08 | - | [PR #438](https://github.com/fossasia/scrum_helper/pull/438) |
| scrum_helper | Added gitee support for generating scrum reports | Open | 2026-03-08 | - | [PR #436](https://github.com/fossasia/scrum_helper/pull/436) |
| eventyay | Fix/navigation flicker | Closed | 2026-02-03 | - | [PR #2233](https://github.com/fossasia/eventyay/pull/2233) |
| eventyay | Fix/dropdown outside click | Closed | 2026-01-26 | - | [PR #2124](https://github.com/fossasia/eventyay/pull/2124) |
| eventyay | Chore/batch dependency updates | Closed | 2026-01-24 | - | [PR #2079](https://github.com/fossasia/eventyay/pull/2079) |
| scrum_helper | feat: add configurable scrum report sections | Closed | 2026-01-23 | - | [PR #319](https://github.com/fossasia/scrum_helper/pull/319) |
| eventyay | docs: remove deprecated repo links | Closed | 2026-01-21 | - | [PR #1993](https://github.com/fossasia/eventyay/pull/1993) |
| eventyay | docs: remove references to deprecated repositories | Closed | 2026-01-21 | - | [PR #1992](https://github.com/fossasia/eventyay/pull/1992) |
| eventyay | fix: add Jinja template for change password view | Closed | 2026-01-20 | - | [PR #1967](https://github.com/fossasia/eventyay/pull/1967) |
| eventyay | fix: clean up and consolidate order form settings | Closed | 2026-01-15 | - | [PR #1839](https://github.com/fossasia/eventyay/pull/1839) |
| scrum_helper | Enhance UI: gray out labels and sections when extension is disabled | Closed | 2026-01-03 | - | [PR #288](https://github.com/fossasia/scrum_helper/pull/288) |
| scrum_helper | Fix/disable UI when extension of | Merged | 2026-01-03 | 2026-01-11 | [PR #286](https://github.com/fossasia/scrum_helper/pull/286) |

### Issues

| Repository | Issue Title | Status | Created At | Closed At | Link |
|------------|-------------|--------|------------|-----------|------|
| scrum_helper | [Enhancement]: UI bug, "Generating..." text overflows button on click | Open | 2026-03-22 | - | [Issue #477](https://github.com/fossasia/scrum_helper/issues/477) |
| scrum_helper | [Enhancement]: Error message overlaps footer in popup UI | Open | 2026-03-08 | - | [Issue #437](https://github.com/fossasia/scrum_helper/issues/437) |
| scrum_helper | [Enhancement]: Gray out section labels and text when extension is disabled | Closed | 2026-01-03 | 2026-02-26 | [Issue #287](https://github.com/fossasia/scrum_helper/issues/287) |

